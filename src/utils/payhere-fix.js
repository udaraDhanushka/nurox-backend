...
class PayHereService {
   ...
   */
  generatePaymentHash(orderId, amount, currency = 'LKR', recurrence = null, duration = null, startupFee = 0) {
    try {
   ...
   */
  verifyNotificationHash(merchant_id, order_id, payhere_amount, payhere_currency, status_code, md5sig) {
    try {
      // STEP 1: Hash the merchant secret and convert to uppercase (CRITICAL FIX)
      const hashedSecret = crypto.createHash('md5')
        .update(this.merchantSecret)
  ...
  generatePaymentFormData(paymentData) {
    try {
      ...
      const hash = this.generatePaymentHash(orderId, amount, currency, recurrence, duration, startupFee);
