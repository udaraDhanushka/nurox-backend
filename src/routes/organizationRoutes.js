...
router.get('/hospitals',
  ...
  [
    query('status').optional().isIn(['ACTIVE', 'INACTIVE', 'PENDING_APPROVAL', 'SUSPENDED']),
...
router.get('/hospitals/:hospitalId/doctor-verification-requests',
  authMiddleware,
  requireRole('SUPER_ADMIN', 'HOSPITAL_ADMIN'),
  [
    param('hospitalId').isLength({ min: 1 }).withMessage('Hospital ID is required'),
    query('status').optional().isIn(['PENDING', 'APPROVED', 'REJECTED', 'NEEDS_REVIEW'])
  ],
  validateRequest,
  organizationController.getDoctorVerificationRequests
);

router.put('/hospitals/:hospitalId/doctors/:doctorId/verification',
  authMiddleware,
  requireRole('SUPER_ADMIN', 'HOSPITAL_ADMIN'),
  [
    param('hospitalId').isLength({ min: 1 }).withMessage('Hospital ID is required'),
    param('doctorId').isLength({ min: 1 }).withMessage('Doctor ID is required'),
    body('status').isIn(['APPROVED', 'REJECTED', 'NEEDS_REVIEW']).withMessage('Valid status is required'),
