...
const prescriptionController = {
  ...
  createPrescription: async (req, res) => {
    try {
      ...
      logger.info(
        `Prescription created: ${prescription.id} by Dr. ${req.user.email}`
      );
  ...
  getPrescriptions: async (req, res) => {
    try {
      ...
      // Debug logging
      logger.info(
        `getPrescriptions called by user: ${req.user.id}, role: ${req.user.role}, email: ${req.user.email}`
      );
      ...
      if (req.user.role === 'PATIENT') {
        ...
        if (statusFilter) {
          where.OR = [
            {
              AND: [{ pharmacistId: req.user.id }, { status: statusFilter }],
            },
            {
              AND: [{ pharmacistId: null }, { status: statusFilter }],
            },
          ];
        } else {
        ...
        logger.info(
          `Pharmacist filter applied: userId = ${req.user.id}, filter:`,
          where.OR
        );
      }
      ...
      logger.info(
        `Query results: found ${prescriptions.length} prescriptions out of ${total} total`
      );
  ...
  dispensePrescriptionItem: async (req, res) => {
    try {
      ...
      logger.info(
        `Prescription item dispensed: ${itemId} by ${req.user.email}`
      );
  ...
  processOCRPrescription: async (req, res) => {
    try {
      const { patientId, imageUrl, ocrText, detectedMedicines, confidence } =
        req.body;
  ...
  getPrescriptionAnalytics: async (req, res) => {
    try {
      ...
      const [total, pending, processing, ready, dispensed, cancelled] =
        await Promise.all([
          prisma.prescription.count({ where }),
