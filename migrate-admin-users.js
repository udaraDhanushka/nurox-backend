...
async function migrateAdminUsers() {
  try {
    ...
    console.log(
      `Successfully migrated ${updateResult.count} users from ADMIN to SUPER_ADMIN`
    );
    ...
    if (remainingAdminUsers === 0) {
      console.log(
        '✅ Migration completed successfully. No ADMIN users remaining.'
      );
    } else {
      console.warn(
        `⚠️  Warning: ${remainingAdminUsers} ADMIN users still remain.`
      );
    }
  } catch (error) {
