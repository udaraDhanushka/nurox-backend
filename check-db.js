...
const client = new Client({
  connectionString: "postgresql://postgres.dksvomenfhqajbatyyvc:Nurox@123@aws-0-ap-southeast-1.pooler.supabase.com:5432/postgres"
});
...
async function checkDatabase() {
  try {
    ...
    const bcryptPasswords = passwordStats.rows.filter(u => bcryptPattern.test(u.password));
    
    console.log(`  Total passwords: ${passwordStats.rows.length}`);
    console.log(`  Bcrypt hashed passwords: ${bcryptPasswords.length}`);
    console.log(`  Other format passwords: ${passwordStats.rows.length - bcryptPasswords.length}`);
    
    if (bcryptPasswords.length > 0) {
      console.log(`  Sample bcrypt hash: ${bcryptPasswords[0].password.substring(0, 30)}...`);
    }
    
    // Check active vs inactive users
    const activeUsers = await client.query('SELECT COUNT(*) FROM users WHERE "isActive" = true');
    const inactiveUsers = await client.query('SELECT COUNT(*) FROM users WHERE "isActive" = false');
